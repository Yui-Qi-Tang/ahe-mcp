...
func decodeBoundedReviewRequest(payload []byte, dest any, label string) error {
	fail := func(message string) error { return &ToolError{Code: toolErrorInvalidRequest, Message: message} }
	...
	if err := decodeStrict(payload, dest); err != nil {
		return fail(err.Error())
	}
	return nil
}
