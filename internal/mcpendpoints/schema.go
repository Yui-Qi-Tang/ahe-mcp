...
func (b *Backend) Tools() []mcpstdio.Tool {
	...
	return []mcpstdio.Tool{
		{Name: ToolReview, Title: "Review Evidence Endpoint", Description: "Reconstruct one endpoint from native DB authority. repository_code requires a parser-grounded repository proposal and forbids derivation; derived_spec requires a source-backed statement proposal plus one to eight sorted admitted AND parent IDs and explicit derivation method, producer, trace_ref. Show the complete display, source context, immutable code and all ancestor leaves to the reviewer. The nested source-basis effect is context, not approval. No writes; does not prove entailment, behavior or currentness.", InputSchema: review, Annotations: mcpstdio.Annotations{ReadOnlyHint: &ro, DestructiveHint: &no, IdempotentHint: &yes}},
